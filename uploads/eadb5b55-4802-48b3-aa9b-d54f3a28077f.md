...
![](%E5%89%8D%E7%AB%AF%E5%BC%80%E5%8F%91%E9%AB%98%E9%A2%91%E9%9D%A2%E8%AF%95%E9%A2%98%E5%90%88%E9%9B%86.assets/html1.webp)
...
如果没有 defer 或 async 属性，浏览器会立即加载并执行相应的脚本。加载 JS 时会阻塞 HTML 解析（阻塞了 DOM 的构建，因为 JS 代码可能会对 DOM 做一些操作，所以需要先执行 JS 代码再构建 DOM）

CSS 也会阻塞 HTML 解析，但他不直接阻塞 DOM 的构建，当 JS 代码中引用了 CSS 或者 CSSOM，则需要等待对应的 CSS 下载完成后再开始运行，所以他会阻塞 JS 的运行，间接也就阻塞了 HTML 解析

下图可以直观的看出三者之间的区别: ![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b0a8a139519f46dfa2d1992c58eb5397~tplv-k3u1fbpfcp-watermark.awebp)
...
### 4.1 prefetch 和 preload 的区别

**Preload**

- 用于加载当前页面所需资源，提高页面加载速度
- 会在页面加载时立即加载资源，不会阻塞页面的加载
- 适用于当前页面需要的资源，如 CSS、JS 等
- 语法：`<link rel="preload" href="style.css" as="style">`

-`as`属性用于指定资源的类型，如 `style`、`script`、`image`等

**Prefetch**

- 用于加载未来页面所需资源，提高未来页面的加载速度
- 会在浏览器空闲时加载资源，不会阻塞当前页面的加载
- 适用于未来页面需要的资源，如下一个页面的 CSS、JS 等
- 语法：`<link rel="prefetch" href="style.css">`
- 适用于提前加载下一个页面的资源，提高用户体验
- 但是如果用户没有访问到下一个页面，那么这个资源就白加载了

**Prerender**

- 预渲染，会在后台加载下一个页面的所有资源，当用户访问下一个页面时，可以直接展示

**Preconnet**

- 预连接，会在后台加载下一个页面的所有资源，但是不会渲染，只是提前建立连接
...
<audiosrc=""controlsautoplayloop="true"></audio>
...
<videosrc=""poster="imgs/aa.jpg"controls></video>
...
<video>
  ...
  <sourcesrc='aa.flv'type='video/flv'></source>

  <sourcesrc='aa.mp4'type='video/mp4'></source>
...
<imgsrc="image-128.png"srcset="image-256.png 2x" />
...
<labelfor="mobile">Number:</label> <inputtype="text"id="mobile" />
